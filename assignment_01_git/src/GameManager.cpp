...
void GameManager::createVAO() {
	...
	program->setAttributePointer("position", 3 , GL_FLOAT, GL_FALSE, sizeof(MyVertex), 0);
	program->setAttributePointer("normal", 3, GL_FLOAT, GL_FALSE, sizeof(MyVertex), (void*)(3*sizeof(float)));
	modelInterleaved->getArray()->unbind();
	CHECK_GL_ERROR();
	*/
	
	model.reset(new Model("models/ball.obj", false));
